-- SPEC v3 §3.6/§4 — `apply_mutations` impara `user_favorites` e `user_ratings` (blocco 9).
--
-- Senza questi rami ogni voto e ogni favorite del client finirebbe in `table_not_handled`,
-- cioe' in `sync_rejected_mutations`: la perdita silenziosa di sempre.
--
-- **`user_favorites`**: upsert sulla PK (utente, media, slot), lastWriteWins. Lo slot svuotato
-- e' una lapide, mai una DELETE fisica (il grant DELETE al client nemmeno esiste).
--
-- **`user_ratings`**: la tabella non ha id sintetico e il suo indice unico e' parziale (solo
-- righe vive), quindi ON CONFLICT non puo' vedere la riga gia' cancellata. Il ramo fa
-- update-or-insert sulla chiave naturale: l'UPDATE rianima la lapide riusando la riga (come il
-- re-follow), l'INSERT nasce solo se della chiave non c'e' traccia. Un insert cieco creerebbe
-- una riga nuova a ogni re-voto dopo cancellazione, e i dispositivi divergerebbero.
--
-- Il cancello d'identita' resta quello di 20260801140000: entrambe le tabelle hanno `user_id`,
-- quindi il confronto e' quello standard, nessun caso nuovo.
--
-- Il resto della funzione e' identico al `prosrc` in produzione, verificato con md5 PRIMA di
-- scrivere questo file (raw 5b942e0d71ea77b0b5efb560c50e5dbd, 27236 caratteri, identico al
-- file 20260801140000): si confronta col `prosrc` reale, non con l'ultimo file in cartella.

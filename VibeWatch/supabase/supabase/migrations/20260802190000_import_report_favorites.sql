-- SPEC v3 §7.1 — i Favorites nel report di fine import, ora che la fase 4 li scrive
-- (import-write v6: righe `row_kind='favorite'` → slot LIBERI di `user_favorites`).
--
-- La grammatica è quella di stati e voti: importati / lasciati stare (slot pieni o serie già
-- favorita: 4 slot sono il prodotto, non una perdita) / non risolti. In più
-- `favorite_film_non_supportati`, letto dai totals della fase 2: i film di TV Time non hanno
-- id risolvibili (solo uuid interni e nomi) e si dichiarano invece di sparire.
--
-- Sostituisce per intero la funzione di `20260802170000_import_report_ratings.sql`;
-- eventi, voti e stati sono identici.
...
comment on function public.import_report(uuid) is
  'SPEC v3 §7.4: il report obbligatorio di fine import — stati per-serie e Favorites (§7.1) '
  'e voti in stelle (§7.5) compresi. security invoker: la visibilita'' la decidono le policy '

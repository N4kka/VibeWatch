-- SPEC v3 §3.6/§4 — `apply_mutations` impara `user_follows`.
--
-- Senza questo ramo ogni follow del client finirebbe in `table_not_handled`, cioe' in
-- `sync_rejected_mutations`: la stessa perdita silenziosa contro cui esiste il ramo del tracking.
--
-- **Il cancello d'identita' cambia, e va detto perche'.** La funzione respinge ogni scrittura il
-- cui record porta uno `user_id` diverso dal chiamante. `user_follows` pero' uno `user_id` non ce
-- l'ha: la sua identita' e' `follower_id` — seguire e' un atto del follower. Il cancello ora
-- confronta la colonna d'identita' della tabella, non un nome fisso; per tutte le altre tabelle
-- il comportamento e' identico a prima, byte per byte.
--
-- **DELETE porta il followee in `id`.** La PK e' la coppia (follower, followee) e il follower e'
-- sempre il chiamante: l'unico id che il client puo' voler dire e' l'altro capo. L'unfollow
-- normale resta comunque un UPSERT con `deleted_at` valorizzato (strategia union, come
-- `user_blocks`).
--
-- Un follow attraverso un blocco lo respinge il trigger `user_follows_blocked` (23514): qui
-- diventa `constraint_23514` in `sync_rejected_mutations` — un rifiuto registrato, non un errore
-- che blocca la coda, e soprattutto non un silenzio.
--
-- Il resto della funzione e' identico al `prosrc` in produzione, verificato con md5 normalizzato
-- (09425e9ea32eb977f49a08b0ec5f1a14) prima di scrivere questo file, come impone la lezione
-- dell'ordine delle migration: si confronta col `prosrc` reale, non con l'ultimo file in cartella.

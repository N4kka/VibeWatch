-- SPEC v3 §7.1/§7.4 — i FILM nel report di fine import, ora che la pipeline li porta
-- (import-parse v7: righe `row_kind='movie'`; import-resolve v10: risoluzione per titolo
-- exact-match+anno via catalog-resolve `movie_titles`; import-write v7: watch_events movie +
-- liste legacy). `film_importati`/`film_supportati` smettono di essere lo 0 hardcoded e
-- diventano conteggi veri; l'elenco dei non risolti porta titolo e motivo, come per serie e
-- stati — §7.4: un import che perde 3 film in silenzio non e' "fatto".
--
-- Un job VECCHIO non rigenera mai il report (si archivia alla finalize), quindi il `true`
-- fisso di `film_supportati` vale solo per i report generati da qui in avanti.
--
-- Sostituisce per intero la funzione di `20260802190000_import_report_favorites.sql`;
-- eventi, voti, stati e favorites sono identici.
...
as $$
  ...
  -- `gia_in_lista` NON e' una perdita (il film c'e' gia', o c'era una lapide scelta in app):
  -- sta fuori da questo elenco. Tutto il resto dei non scritti si dichiara col titolo.
  film_non_risolti as (
  ...
  select jsonb_build_object(
    ...
    -- §7.1: i film di v1. `film_importati` = visioni scritte in watch_events;
    -- la watchlist e i lasciati stare contano a parte, ciascuno col proprio nome.
    ...
    -- §7.1: i Favorites. Stessa distinzione dei job vecchi via `favorites_supportati`.
...
comment on function public.import_report(uuid) is
  'SPEC v3 §7.4: il report obbligatorio di fine import — stati per-serie, Favorites e FILM '
  '(§7.1) e voti in stelle (§7.5) compresi. security invoker: la visibilita'' la decidono le policy '
  'di import_jobs/import_staging.';

-- Redesign 2.0, quarto giro — chi esce dall'inbox non sparisce dal report.
--
-- `import_exclude_unresolved` marca le righe con `error = 'escluso: utente'`, e finora la
-- funzione le toglieva dal report: un titolo escluso durante la verifica spariva senza traccia
-- e l'utente non aveva più modo di sapere che era stato lasciato fuori. Ora torna nell'elenco
-- "rimasti fuori" con il suo motivo, MA fuori dai contatori: i contatori guidano l'inbox e il
-- banner, e un titolo escluso non è lavoro rimasto da fare.
--
-- Secondo cambiamento: i film non risolti portano `anno` e `visto_il`. L'export li ha
-- (`release_year`, `happened_at`), il report li buttava via, e la card del report non poteva
-- dire nulla oltre al titolo.
--
-- Sostituisce per intero la funzione di `20260804150000_import_report_fuori_struttura.sql`;
...
as $$
  ...
  non_riconosciuti as (
    select coalesce(raw->>'series_name', '(senza titolo)') as titolo,
           ...
           -- Il motivo vero ha la precedenza sull'esclusione: dice PERCHE' era finito qui.
           coalesce(
  ...
  film_non_risolti as (
    select coalesce(raw->>'title', '(senza titolo)') as titolo,
           ...
           coalesce(
             ...
             'motivo non registrato')                as motivo,
           bool_and(coalesce(error, '') = 'escluso: utente') as escluso,
           -- L'export li ha: senza, la card del report non sa dire quale film sia.
  ...
  select jsonb_build_object(
    ...
    -- I contatori guidano inbox e banner: gli esclusi non sono lavoro rimasto.
    ...
    -- L'elenco invece li tiene, marcati: il report deve poter dire "questo l'hai escluso tu".
...
comment on function public.import_report(uuid) is
  'SPEC v3 §7.4 + redesign 2.0: report di fine import. Gli esclusi dall''utente restano '
  'nell''elenco marcati (escluso = true) ma fuori dai contatori; i fuori-struttura TMDB hanno '
  'i loro campi dichiarativi. security invoker: decide la RLS.';
